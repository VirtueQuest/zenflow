...
#  IMPORTS
# ─────────────────────────────────────────────────────────────
import sqlite3, os, random, string, re, time, logging, json, hashlib, secrets
...
#  RATE LIMITER
# ─────────────────────────────────────────────────────────────
limiter = Limiter(key_func=get_remote_address, default_limits=[RL_DEFAULT])
...
# Phase 4 — Prometheus metrics middleware (innermost = fastest path)
...
# ─────────────────────────────────────────────────────────────
#  PASSWORD HASHING  (bcrypt + timing-safe compare)
# ─────────────────────────────────────────────────────────────
...
def verify_password(plain: str, hashed: str) -> bool:
    if hashed == "h_demo":          # seed data legacy
        import hmac
...
async def startup():
    ...
    async def _warm():
        async for db in get_db():
            await warm_cache(db)
            # Snapshot business gauges
            if PROM_OK:
                try:
                    n_pros = await db.fetchval(
                        "SELECT COUNT(*) FROM professionals WHERE is_available=TRUE" if DATABASE_URL
                        else "SELECT COUNT(*) FROM professionals WHERE is_available=1"
                    )
                    n_ads  = await db.fetchval(
...
def list_professionals(
    ...
    if q:
        ...
        if words:
            ...
            for w in words:
                clauses.append(
                    "(LOWER(p.display_name)||' '||LOWER(COALESCE(p.title,''))||' '"
                    "||LOWER(COALESCE(p.bio,''))||' '||LOWER(COALESCE(p.location,'')) LIKE ?)"
                )
                wparams.append(f"%{w}%")
