-- ═══════════════════════════════════════════════════════════════════
--  ZenFlow — MySQL Schema
--  Converted from PostgreSQL to MySQL
--
--  Run:
--  mysql -u zenflow -p zenflow < 001_mysql_schema.sql
--  # Enter password: Qwerty12$12
-- ═══════════════════════════════════════════════════════════════════
...
-- Recalculate professional rating when review inserted/updated
DELIMITER //

CREATE TRIGGER trg_rating_on_review_insert
...
CREATE TRIGGER trg_rating_on_review_update
...
-- Auto-expire ads when days_left = 0
CREATE TRIGGER trg_expire_ad
...
-- Deduct tokens + log transaction when ad created
CREATE TRIGGER trg_token_spend_on_ad
...
-- Credit tokens + log when payment completed
CREATE TRIGGER trg_token_credit_on_payment_insert
...
CREATE TRIGGER trg_token_credit_on_payment_update
