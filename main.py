...
async def validation_error_handler(request: Request, exc: RequestValidationError):
    ...
    for err in exc.errors():
        ...
        msg   = msg.replace("Value error, ", "").replace("String should have at least", "Must be at least").replace("String should have at most", "Must be at most")
        messages.append(f"{field}: {msg}")
        # Build a fully JSON-safe error dict (no Python objects)
        clean_errors.append({"field": field, "message": msg, "type": str(err.get("type", ""))})
    return _JSONResponse(
        status_code=422,
        content={"detail": clean_errors, "messages": messages, "summary": " • ".join(messages)}
    )
